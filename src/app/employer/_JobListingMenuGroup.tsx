...
type JobListing = Pick<typeof JobListingTable.$inferInsert, "title" | "id"> & {
    applicationCount: number
}

export function JobListingMenuGroup({ status, jobListings }: { status: JobListingStatuses, jobListings: JobListing[] }) {
    const { jobListingId } = useParams()

    return <SidebarMenu>
        <Collapsible
            defaultOpen={status !== "delisted" || jobListings.find(j => j.id === jobListingId) != null}
...
function JobListingMenuItem({ id, title, applicationCount }: JobListing) {
    ...
    return <SidebarMenuSubItem>
        ...
        {applicationCount > 0 &&
            <div className="absolute right-2 top-1/2 -translate-y-1/2 text-sm text-muted-foreground">{applicationCount}</div>
        }
    </SidebarMenuSubItem>
}