...
CREATE TABLE "user_notifications_settings" (
	...
	"newJobEmailNotifications" boolean DEFAULT true NOT NULL,
	"aiPrompt" varchar,
	"createdAt" timestamp with time zone DEFAULT now() NOT NULL,
	"updatedAt" timestamp with time zone DEFAULT now() NOT NULL
);
...
CREATE TABLE "job_listings" (
	...
	"locationRequirement" "job_listings_location_req" NOT NULL,
...
--> statement-breakpoint
ALTER TABLE "user_notifications_settings" ADD CONSTRAINT "user_notifications_settings_userId_users_id_fk" FOREIGN KEY ("userId") REFERENCES "public"."users"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
