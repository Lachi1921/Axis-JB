...
const listingSchema = z.object({
    ...
    expierenceLevel: z.enum(expierenceLevels),
    type: z.enum(jobType),
...
export async function getMatchingJobListings(
    ...
    const agent = createAgent({
        name: "Job Listing Matcher",
        description: "Agent for matching users with job listings",
        system: `You're an expert at matching people with jobs based on their specific experience, and requirments. The provided user prompt will be a description that can include information about themselves as well what they are looking for in a job. ${max ? `Only return a maximum of ${max} jobs.` : "Return all jobs that match their requirements."} Return the jobs as a comma seperated list of jobIds. If you cannot find any jobs that match the user prompt, return the text "${NO_JOBS}". Here is the JSON array of available job listings: ${JSON.stringify(
            jobListings.map(listing => listingSchema.transform(listing => ({
                ...listing,
